...
func (s *Service) GetDutySettings(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID) (*dto.DutySettingsResponse, error) {
	...
	activeDuty, err := s.dutyRepo.FindActiveByGroupID(ctx, group.ID(), s.now())
	if err != nil {
		return nil, fmt.Errorf("load active duty: %w", err)
	}
	canManageParticipants := canManageGroupSettings || canManageDormitory || (activeDuty != nil && activeDuty.LeaderID() != nil && *activeDuty.LeaderID() == currentUserID)
	...
	responseAreas, activeDutyResponse, taskEditorState, taskEditorAlert, err := s.buildTaskEditorState(
		ctx,
		group,
		tasks,
		lastCompletionDates,
	...
	if canManageGroupSettings {
		responseTeams, activeDutyTeamID, err = s.buildDutySettingsTeams(ctx, group, activeDuty)
		if err != nil {
			return nil, err
		}
	} else {
...
func (s *Service) GetTeamMembers(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, teamID uuid.UUID) (*dto.DutySettingsTeamMembersResponse, error) {
	...
	sort.Slice(members, func(i, j int) bool {
		leftIsLeader := team.LeaderID() == members[i].ID()
		rightIsLeader := team.LeaderID() == members[j].ID()
	...
	for _, member := range members {
		responseMembers = append(responseMembers, dto.DutySettingsTeamMember{
			...
			IsLeader: team.LeaderID() == member.ID(),
		})
	}

	return &dto.DutySettingsTeamMembersResponse{
		TeamName: userSummaryFromMap(residentNames, team.LeaderID()).Name,
		Leader:   userSummaryFromMap(residentNames, team.LeaderID()),
...
func (s *Service) AddTeamMember(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, teamID uuid.UUID, userID uuid.UUID) error {
	...
	if resident.TeamID() != nil {
		...
		if sourceTeam.LeaderID() == resident.ID() {
...
func (s *Service) AssignTeamLeader(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, teamID uuid.UUID, userID uuid.UUID) error {
	...
	updated := structure.RestoreTeam(team.ID(), team.GroupID(), userID, team.Color(), team.RotationPosition())
...
func (s *Service) CreateTeam(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, req dto.CreateResidentTeamRequest) (*dto.TeamDetails, error) {
	...
	if req.GroupID != "" {
		...
		if err != nil || parsedGroupID != groupID {
	...
	if err := s.requireUsersInDormitory(ctx, group.DormitoryID(), []uuid.UUID{leaderID}); err != nil {
		return nil, err
	}
...
func (s *Service) UpdateTeam(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, teamID uuid.UUID, req dto.UpdateResidentTeamRequest) (*dto.TeamDetails, error) {
	...
	updated := structure.RestoreTeam(teamID, groupID, leaderID, team.Color(), team.RotationPosition())
	if err := s.teamRepo.Save(ctx, updated); err != nil {
		return nil, err
	}
...
func (s *Service) AssignActiveDutyTeam(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, teamID uuid.UUID) error {
	...
	activeDuty, err := s.requireActiveDuty(ctx, groupID)
	if err != nil {
		return err
	}
...
func (s *Service) CreateTask(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, areaID int, req dto.CreateTaskRequest) (*dto.TaskDetails, error) {
	...
	if normalized.includeInCurrentDuty {
		activeDuty, err := s.requireActiveDuty(ctx, group.ID())
		if err != nil {
			return nil, err
		}
	...
	return buildTaskDetails(task, area), nil
}
...
func (s *Service) UpdateTask(ctx context.Context, currentUserID uuid.UUID, groupID uuid.UUID, taskID uuid.UUID, req dto.UpdateTaskRequest) (*dto.TaskDetails, error) {
	...
	return buildTaskDetails(updated, targetArea), nil
}
...
func (s *Service) buildDutySettingsTeams(ctx context.Context, group *structure.Group, activeDuty *duty.Duty) ([]dto.DutySettingsTeam, *string, error) {
	teams, err := s.teamRepo.FindByGroupID(ctx, group.ID())
	if err != nil {
	...
	activeDutyTeamID := activeDuty.TeamID().String()
...
func (s *Service) buildTaskEditorState(
	...
	team, err := s.teamRepo.FindByID(ctx, activeDuty.TeamID())
	if err != nil {
	...
	residents, err := s.userRepo.FindByDormitoryID(ctx, group.DormitoryID())
	if err != nil {
		return nil, nil, "", "", fmt.Errorf("load duty residents: %w", err)
	}
	...
	dutyTasksByTaskID := make(map[uuid.UUID]*duty.DutyTask, len(activeDuty.Tasks()))
	for _, dutyTask := range activeDuty.Tasks() {
		dutyTasksByTaskID[dutyTask.TaskDefID()] = dutyTask
	}

	assigneeNames := make(map[uuid.UUID]string, len(residents))
	for _, resident := range residents {
		assigneeNames[resident.ID()] = fullUserName(resident)
	}
	...
	activeParticipants, err := s.participantRepo.List(ctx, activeDuty.ID(), false)
	if err != nil {
	...
	return responseAreas, &dto.DutySettingsActiveDuty{
		ID:        activeDuty.ID().String(),
		TeamID:    team.ID().String(),
		TeamName:  dutyLeaderName(ctx, s.userRepo, activeDuty.LeaderID()),
		StartDate: activeDuty.Start().Format("2006-01-02"),
		EndDate:   activeDuty.End().Format("2006-01-02"),
...
func dutyLeaderName(ctx context.Context, repository user.Repository, leaderID *uuid.UUID) string {
	if leaderID == nil {
		return ""
	}
	leader, err := repository.FindByID(ctx, *leaderID)
	if err != nil || leader == nil {
		return ""
	}
	return fullUserName(leader)
}
...
func (s *Service) getTeamMemberItems(ctx context.Context, dormitoryID int64, currentTeamID uuid.UUID) ([]dto.TeamMemberItem, error) {
	...
	teamLeaders := make(map[uuid.UUID]uuid.UUID)
	for _, group := range allGroups {
		...
		for _, team := range groupTeams {
			teamNames[team.ID()] = userSummaryFromMap(userNames(users), team.LeaderID()).Name
			teamLeaders[team.ID()] = team.LeaderID()
		}
	}
	...
	for _, resident := range users {
		...
		items = append(items, dto.TeamMemberItem{
			...
			IsTeamLeader:    resident.TeamID() != nil && teamLeaders[*resident.TeamID()] == resident.ID(),
		})
	}
...
func (s *Service) requireActiveDuty(ctx context.Context, groupID uuid.UUID) (*duty.Duty, error) {
	activeDuty, err := s.dutyRepo.FindActiveByGroupID(ctx, groupID, s.now())
	if err != nil {
		return nil, fmt.Errorf("load active duty: %w", err)
	}
	if activeDuty == nil {
...
func buildTaskDetails(task *catalog.TaskDefinition, area *catalog.Area) *dto.TaskDetails {
