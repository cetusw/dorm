...
Далее состав не зависит от переводов между командами: жителя доступного scope можно
...
На странице настроек вкладка «Исполнители» управляет только фактическим составом
календарно текущего Duty. Она не меняет `team`, `team.leader_id` или `user.team_id`.
Исключённого участника можно восстановить: исходный тип `REGULAR`/`TEMPORARY`
сохраняется. При исключении фактического лидера `duty.leader_id` новая активная
замена назначается в той же атомарной операции.

Пока у Resident нет самостоятельной устойчивой принадлежности к Group, кандидаты
для временного добавления ограничены активными жителями того же Dormitory. После
появления такой принадлежности этот scope должен быть сужен до Group.

При ручной смене дежурной Team у календарно текущего Duty состав также заменяется
в одной транзакции: прежние активные участники получают `excluded_at`, а текущие
члены и лидер новой Team становятся активными `REGULAR` участниками. `duty.leader_id`
обновляется на лидера новой Team.
...
1. читает все группы;
2. пропускает группы без active Team и подбирает команду для остальных;
...
Если команда последнего Duty больше не active, rotation anchor ищется в последнем более
раннем Duty active Team. Если такого Duty нет, выбирается первая Team текущей ротации.
`sequence_number` при этом всегда продолжает полную историю группы, включая Duty удалённых Team.
Ручное создание Duty для группы без Team возвращает ошибку; batch scheduler такую группу пропускает.
...
Цель по баллам округляется вверх от общей суммы стоимости задач на число active
`duty_participants` конкретного Duty.
...
Если у жителя есть сохранённое участие в незавершённом прошлом Duty, экран `/app/tasks`
показывает именно его, даже если его постоянная Team была удалена или он больше не состоит
в команде.
