...
- Permissions:
  - только фактический глава конкретного Duty (`duty.leader_id`)
...
`DELETE` существующих management и duty-settings Team routes выполняет необратимое
soft delete и возвращает `204`. Команда исчезает из текущих списков, а её постоянные
участники больше не состоят в ней; история Duty сохраняется. Если Team участвует в
календарно текущем Duty, route возвращает `409`.
...
### `DELETE /api/v1/groups/:id/duty-settings/teams/:teamId/members/:userId`

- Назначение: исключить участника постоянной команды.
- Body: опциональный `{ "replacement_leader_id": "<uuid>" }`.
- Для обычного участника body не нужен. При исключении текущего главы replacement
  обязателен, должен быть другим активным участником этой же команды; назначение
  replacement и исключение выполняются атомарно.
- Единственного главы команды исключить нельзя.

### `POST /api/v1/groups/:id/duty-settings/teams`

- Body: `{ "leader_id": "<uuid>" }`.
- Команда не имеет собственного имени: её отображаемое название — полное имя главы.
- Новый глава становится первым участником команды в той же транзакции; новая команда
  добавляется в конец ротации. Главу другой команды выбрать нельзя.
- Ошибки: `400` для malformed UUID/body, `403` для отсутствия доступа, `409` для
  недопустимого replacement или состояния команды.
